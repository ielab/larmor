...
import model.contriever.src.dist_utils as dist_utils
import model.contriever.src.normalize_text as normalize_text
from model.model_collection import CustomDEModel
from model.contriever.src.contriever import Contriever
import transformers


class DenseEncoderContrieverModel(CustomDEModel):
    def  __init__(self, max_length=512, add_special_tokens=True, norm_query=False, norm_doc=False,
                  lower_case=False, normalize_text=False, cuda=True, cache_dir="./", **kwargs):
        super().__init__(**kwargs)

        config = transformers.AutoConfig.from_pretrained( pretrained_model_name_or_path="facebook/contriever-msmarco",
                                                          cache_dir=cache_dir)
        tokenizer = transformers.AutoTokenizer.from_pretrained("facebook/contriever-msmarco",
                                                                    cache_dir=cache_dir)
        retriever = Contriever.from_pretrained("facebook/contriever-msmarco",
                                                    cache_dir=cache_dir)
        ...
        query_encoder.eval()
        # print("Query encoder", query_encoder, next(query_encoder.parameters()).is_cuda)
        if doc_encoder is not None:
            if hasattr(doc_encoder, "module"):
                doc_encoder = doc_encoder.module
            doc_encoder.eval()
        else:
            # Document encoder and query encoder are the same
    ...
    def tokenize_corpus(self, corpus: List[Dict[str, str]], **kwargs):
        idx = range(len(corpus))
        corpus = [corpus[i] for i in idx]
        corpus = [c["title"] + " " + c["text"] if len(c["title"]) > 0 else c["text"] for c in corpus]
